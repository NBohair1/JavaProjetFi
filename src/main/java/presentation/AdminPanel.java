...
public class AdminPanel extends JPanel {
            ...
            try {
                ...
                for (dao.Reparateur rep : reparateurs) {
                    ...
                    for (dao.Emprunt emp : emprunts) {
                        String etat = emp.isRembourse() ? "Rembourse" : "En cours";
    ...
    private JPanel createGestionAdministrativePanel() {
        ...
        JLabel lblTitle = new JLabel("Administration Avancee");
        ...
        JButton btnDetailsReparations = createAdminButton("Details Caisses + Reparations", new Color(52, 152, 219));
        JButton btnDernieresRep = createAdminButton("Dernieres Reparations", new Color(46, 204, 113));
        JButton btnGestionRep = createAdminButton("Gestion CRUD Reparations", new Color(241, 196, 15));
        JButton btnGestionReparateurs = createAdminButton("Gestion des Reparateurs", new Color(231, 76, 60));
    ...
    private void afficherToutesCaisses() {
        try {
            // Lister toutes les boutiques du propriétaire
            ...
            if (boutiques == null || boutiques.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Aucune boutique disponible", "Info", JOptionPane.INFORMATION_MESSAGE);
                return;
            }

            // Rassembler tous les réparateurs de toutes les boutiques
            java.util.List<Reparateur> tousReparateurs = new java.util.ArrayList<>();
            for (dao.Boutique b : boutiques) {
                List<Reparateur> reps = boutiqueMetier.listerReparateurs(b);
                if (reps != null) tousReparateurs.addAll(reps);
            }

            if (tousReparateurs.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Aucun réparateur trouvé", "Info", JOptionPane.INFORMATION_MESSAGE);
                return;
            }

            // Rassembler toutes les caisses distinctes
            java.util.List<dao.Caisse> toutesCaisses = new java.util.ArrayList<>();
            java.util.Set<Long> idsVus = new java.util.HashSet<>();
            for (Reparateur rep : tousReparateurs) {
                if (rep.getCaisse() != null && rep.getCaisse().getIdCaisse() != null && !idsVus.contains(rep.getCaisse().getIdCaisse())) {
                    toutesCaisses.add(rep.getCaisse());
                    idsVus.add(rep.getCaisse().getIdCaisse());
                }
            }

            if (toutesCaisses.isEmpty()) {
            ...
            String[] cols = {"ID Caisse", "Caisse Réelle (DH)", "Réparateur", "Boutique"};
            DefaultTableModel model = new DefaultTableModel(cols, 0) {
                @Override
                public boolean isCellEditable(int row, int column) { return false; }
            };
            ...
            for (Reparateur rep : tousReparateurs) {
                dao.Caisse caisse = rep.getCaisse();
                if (caisse == null) continue;
                float caisseReelle = caisse.getSoldeActuel();
                try {
                    caisseReelle = caisseMetier.consulterCaisseReelle(rep);
                } catch (Exception e) {
                    // garder valeur par défaut
                }
                String boutiqueNom = rep.getBoutique() != null ? rep.getBoutique().getNom() : "N/A";
                model.addRow(new Object[]{
                    caisse.getIdCaisse(),
                    String.format("%.2f", caisseReelle),
                    rep.getPrenom() + " " + rep.getNom(),
                    boutiqueNom
                });
            }
    ...
    private void afficherStatistiquesGlobales() {
        try {
            ...
            sb.append("Total Reparations : ").append(totalReparations).append("\n");
