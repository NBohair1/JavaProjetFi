...
public class ComposantMetierImpl implements IComposantMetier {
    ...
    public void modifierComposant(Composant composant) throws ComposantException {
        ...
        try {
            ...
            if (tx.isActive()) tx.rollback();
            throw new ComposantException("Erreur lors de la modification du composant");
        } finally {
            em.close();
        }
    }
    ...
    public void supprimerComposant(Long id) throws ComposantException {
        ...
        try {
            ...
            if (tx.isActive()) tx.rollback();
            throw new ComposantException("Erreur lors de la suppression du composant");
        } finally {
            em.close();
        }
    }
    ...
    public List<Composant> listerComposantsParBoutique(Boutique boutique) throws ComposantException {
        if (boutique == null) {
            throw new ComposantException("Boutique non selectionnee");
        }
    ...
    public void utiliserComposant(Composant composant, int quantite) throws ComposantException {
        if (composant == null) {
            throw new ComposantException("Composant non spécifié");
        }
        
        if (quantite <= 0) {
            throw new ComposantException("La quantité doit être positive");
        }
        
        if (composant.getQuantite() < quantite) {
            throw new ComposantException("Stock insuffisant. Disponible: " + composant.getQuantite());
        }
        ...
        try {
            ...
            // Récupérer le composant depuis la base de données
            Composant c = em.find(Composant.class, composant.getIdComposant());
            if (c == null) {
                throw new ComposantException("Composant introuvable");
            }
            
            // Décrémenter la quantité
            c.setQuantite(c.getQuantite() - quantite);
            em.merge(c);
            ...
            if (tx.isActive()) tx.rollback();
            throw new ComposantException("Erreur lors de l'utilisation du composant: " + e.getMessage());
        } finally {
            em.close();
        }
    }
